"""Materialize the non-authorizing Qwen 3.8 recovery protocol and full-document pin."""
...
from rejudge import phase3_v3_recovery_materialization as recovery  # noqa: E402
...
def _load_object(path: Path) -> dict:
    ...
    if not isinstance(value, dict):
        raise recovery.RecoveryMaterializationError(f"{path} must contain a JSON object")
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=REPO_ROOT / recovery.DEFAULT_PROTOCOL_OUTPUT_PATH)
    parser.add_argument(
        "--pin-output", type=Path,
        default=REPO_ROOT / recovery.DEFAULT_PROTOCOL_PIN_OUTPUT_PATH)
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args(argv)
    protocol_output = args.protocol_output.resolve()
    pin_output = args.pin_output.resolve()
    prior, amendment, halt = recovery.load_materialization_inputs(REPO_ROOT)
    protocol = recovery.materialize_recovery_protocol(
    ...
    if args.check:
        if _load_object(protocol_output) != protocol or _load_object(pin_output) != pin:
            raise recovery.RecoveryMaterializationError(
                "existing recovery protocol or pin differs from deterministic output")
    else:
        if protocol_output.exists() or pin_output.exists():
            raise recovery.RecoveryMaterializationError(
                "refusing to overwrite an existing recovery protocol or pin")

"""Materialize the non-authorizing Llama-checker recovery4 protocol and pin."""
...
from rejudge import phase3_v3_recovery4_materialization as recovery4  # noqa: E402
...
def _load_object(path: Path) -> dict:
    ...
    if not isinstance(value, dict):
        raise recovery4.Recovery4MaterializationError(f"{path} must contain a JSON object")
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=REPO_ROOT / recovery4.DEFAULT_PROTOCOL_OUTPUT_PATH)
    parser.add_argument(
        "--pin-output", type=Path,
        default=REPO_ROOT / recovery4.DEFAULT_PROTOCOL_PIN_OUTPUT_PATH)
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args(argv)
    protocol_output = args.protocol_output.resolve()
    pin_output = args.pin_output.resolve()
    prior, amendment, stop = recovery4.load_materialization_inputs(REPO_ROOT)
    protocol = recovery4.materialize_recovery4_protocol(
        prior, amendment, stop, project_root=REPO_ROOT)
    ...
    if args.check:
        if _load_object(protocol_output) != protocol or _load_object(pin_output) != pin:
            raise recovery4.Recovery4MaterializationError(
                "existing recovery4 protocol or pin differs from deterministic output")
    else:
        if protocol_output.exists() or pin_output.exists():
            raise recovery4.Recovery4MaterializationError(
                "refusing to overwrite an existing recovery4 protocol or pin")

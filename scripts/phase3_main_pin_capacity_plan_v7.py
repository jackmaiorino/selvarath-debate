"""Pin the materialized v7 capacity plan's canonical hash into the preflight validator.

Offline only. Reads the tracked v7 plan, prints its canonical and raw SHA-256, and with
...
PLAN_PATH = (
    REPO_ROOT / "rejudge" / "phase3_main_review_capacity_preflight_plan_v7_2026-09-07.json"
)
VALIDATOR_PATH = REPO_ROOT / "scripts" / "phase3_main_review_capacity_preflight.py"
PLACEHOLDER = 'EXPECTED_PLAN_CANONICAL_SHA256_V7 = "pending-v7-plan-materialization"'
_PINNED_RE = re.compile(
    r'^EXPECTED_PLAN_CANONICAL_SHA256_V7 = "([0-9a-f]{64})"$', flags=re.MULTILINE
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if plan.get("schema_version") != "phase3_main_review_capacity_preflight_plan_v7":
        parser.error("plan is not a v7 capacity preflight plan")
    ...
    if pinned is not None:
        if pinned.group(1) == canonical:
            ...
            return 0
        parser.error("validator already pins a different v7 hash; refusing to overwrite")
    if source.count(PLACEHOLDER) != 1:
        parser.error("placeholder constant not found exactly once in the validator")
    updated = source.replace(
        PLACEHOLDER, f'EXPECTED_PLAN_CANONICAL_SHA256_V7 = "{canonical}"'
    )

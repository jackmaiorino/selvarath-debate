...
if ($Mode -ne 'status' -and ($config.paid_execution_authorized -isnot [bool] -or $config.paid_execution_authorized -ne $true -or $config.stage -ne '5-evidence-scope')) { throw 'A bounded Phase 5 evidence-scope authorization is required.' }
...
if ($Mode -eq 'status') {
    ...
    try {
        & $pythonPath -B -m rejudge.phase5_runner --authorization $authorizationPath --inputs $inputPath --run-dir $runDirectory --mode status
...
$runnerArgs = Quote-Arguments @('-B','-m','rejudge.phase5_runner','--authorization',$authorizationPath,'--inputs',$inputPath,'--run-dir',$runDirectory,'--mode',$Mode)

...
| Owner signing identity | One public-key candidate and fingerprint are identified. Capacity and main authorization share the still-unpinned `rejudge/phase3_owner_signing.py` configuration. No private key is stored or accessible in the repository. | Jack selects the fingerprint, then the single public key configuration is pinned in a clean commit. |
...
| Provider billing reconciliation | Not materialized. Two strict read-only checks confirm `/v1/whoami` for the proposed account while `/v1/billing/usage` returns the organization-not-enabled HTTP 404. The second check published zero files. | Submit `reports/2026-08-30-together-billing-usage-enablement-request.md`; after Together enables the endpoint, capture and reconcile the ratified source set. |

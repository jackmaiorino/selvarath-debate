...
main run halted on it, at event 17022, after 2,573 cells: 5,666 completion tokens against a
4,096 allowance, finish_reason "stop" rather than "length". Over the run so far, 72 of 2,772
Qwen3.7-Plus calls exceeded 4,096, reaching 6,753; every other model's maximum was under it.

Most survived only because the byte-based prompt estimate over-reserves heavily (7,476
against an actual 1,576). This call had a short prompt, so that padding was not there.
"""
from rejudge.api_client import COMPLETION_RESERVE_MULTIPLIER, reserved_completion_tokens
...
def test_the_reservation_allows_for_reasoning_beyond_max_tokens():
    ...
    assert reserved_completion_tokens(4096) >= 6753, (
        "must cover the largest completion observed in the live run")
...
def test_the_context_guard_is_unaffected():
    ...
    prompt, completion = _estimate_usage(messages, 4096)

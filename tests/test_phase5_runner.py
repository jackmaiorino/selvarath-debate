...
from rejudge import phase5_runner as r
...
LIMITS = {"total": r.money(200), "uncertain": r.money(25), "preflight": r.money(5)}
...
def panel(units=1):
    ...
    for index in range(units):
        ...
        for arm in r.ARMS:
            ...
            if prompt == "scope":
                messages[0]["content"] += r.scope_suffix()
            ...
            for judge in r.MODELS:
                calls.append({"cell_id": uid + ":" + judge + ":" + arm, "unit_id": uid, "judge": judge, "arm": arm,
                              ...
                              "context_arm": context, "prompt_variant": prompt,
...
def stores(tmp_path):
    ...
    def make(name="run", limits=None, settings=None):
        value = r.Store(tmp_path / name, {"run_id": r.RUN_ID, "stage": r.STAGE},
...
def test_completed_invalid_verdicts_are_final_and_equal_packets_still_get_separate_calls(stores, text):
    calls, packets, _ = panel()
    assert len({r.canonical(p["messages"]) for p in packets.values()}) == 2
    store, seen = stores(), []
...
def test_single_run_lock_refuses_other_process(tmp_path):
    code = "from pathlib import Path; from rejudge.phase5_runner import run_lock; import sys\ntry:\n with run_lock(Path(sys.argv[1])): sys.exit(7)\nexcept OSError:\n sys.exit(0)"
...
def prepared(tmp_path, monkeypatch):
    frozen_protocol = (r.ROOT / "rejudge/phase5_protocol_v1.json").read_bytes()
    project = tmp_path / "project"
    (project / "rejudge").mkdir(parents=True)
    protocol = project / "rejudge/phase5_protocol_v1.json"
    protocol.write_bytes(frozen_protocol)
    ...
    monkeypatch.setattr(r, "EXPECTED_UNITS", len(units))
    auth = {"schema_version": "phase5_execution_v1", "stage": r.STAGE, "run_id": r.RUN_ID, "paid_execution_authorized": True,
            "approved_cap_usd": 200, "uncertain_cap_usd": 25, "preflight_cap_usd": 5,
    ...
    data = {"calls.jsonl": calls, "packets.jsonl": list(packets.values()), "units_private.jsonl": units, "prompt_edits_private.jsonl": []}
    for name, rows in data.items():
        (inputs / name).write_text("".join(r.canonical(row) + "\n" for row in rows), encoding="utf-8")
    manifest = {"schema_version": "phase5_evidence_scope_prepared_panel_v1", "main_verdict_calls": len(calls), "models": list(r.MODELS),
                "protocol_sha256": auth["protocol_sha256"], "call_seed_namespace": r.SEED_NAMESPACE, "arms": list(r.ARMS),
                "order_seed": r.ORDER_SEED, "analysis_seed": r.ANALYSIS_SEED,
...
def test_authorized_config_cannot_change_frozen_profile(prepared, mutation):
    p = prepared
    if mutation == "protocol": (p.project / "rejudge/phase5_protocol_v1.json").write_text("changed")
...
def test_bound_panel_reopens_and_changed_input_refuses(prepared):
    ...
    with (p.inputs / "prompt_edits_private.jsonl").open("a") as stream:
        stream.write("\n")
    with pytest.raises(ValueError, match="input changed"):
        r.load_panel(p.inputs, p.auth)


def test_formal_authorization_requires_exactly_7872_verdicts(prepared, monkeypatch):
    p = prepared
    monkeypatch.setattr(r, "EXPECTED_MAIN_CALLS", 7872)
    with pytest.raises(ValueError, match="exactly 7872"):
        r.verify_authorization(p.authorization, p.inputs, p.output)
    p.auth["main_verdict_calls"] = 7872
    r.atomic_json(p.authorization, p.auth)
    assert r.verify_authorization(p.authorization, p.inputs, p.output)["main_verdict_calls"] == 7872
...
def test_cli_end_to_end_and_completed_restart_require_no_provider(prepared, monkeypatch, bad_preflight):
    ...
    if bad_preflight:
        ...
        with pytest.raises(ValueError, match="preflight has not passed"):
            r.main(args(p, "run"))
        return
    assert r.main(args(p, "run")) == 0 and len(seen) == 18
    ...
    assert json.loads((p.output / "completion.json").read_bytes())["next_stage_paid_authorized"] is False
...
def test_hidden_launcher_is_valid_powershell_and_rejects_unapproved_before_mutation(tmp_path):
    path = Path(__file__).resolve().parents[1] / "scripts/phase5_launch.ps1"
    authorization = tmp_path / "unapproved.json"
    output = tmp_path / "output"
    authorization.write_text(json.dumps({"paid_execution_authorized": False, "stage": r.STAGE,
    ...
    assert result.returncode != 0 and b"evidence-scope authorization is required" in result.stderr
    assert not output.exists()
    text = path.read_text(encoding="utf-8")
    assert "rejudge.phase5_runner" in text and "rejudge.phase4b_runner" not in text
...
def test_launcher_status_is_readonly_from_outside_repository(tmp_path):
    launcher = Path(__file__).resolve().parents[1] / "scripts/phase5_launch.ps1"
    inputs, output = tmp_path / "inputs", tmp_path / "output"
    inputs.mkdir()
    authorization = tmp_path / "draft.json"
    authorization.write_text(json.dumps({"schema_version": "phase5_execution_v1", "paid_execution_authorized": False, "approved_cap_usd": None,
        "stage": r.STAGE, "run_id": r.RUN_ID, "inputs": str(inputs), "run_directory": str(output)}))
...
def rewrite_bound_panel(p, calls, packets):
    for name, rows in (("calls.jsonl", calls), ("packets.jsonl", list(packets.values()))):
        path = p.inputs / name
        path.write_text("".join(r.canonical(row) + "\n" for row in rows), encoding="utf-8")
        p.manifest["outputs"][name] = {"sha256": r.sha(path), "bytes": path.stat().st_size}
    r.atomic_json(p.inputs / "manifest.json", p.manifest)
    p.auth["input_manifest_sha256"] = r.sha(p.inputs / "manifest.json")


def rehash_packets(calls, packets):
    for packet in packets.values():
        packet["messages_sha256"] = r.recovery.digest(packet["messages"])
    for call in calls:
        call["messages_sha256"] = packets[call["packet_id"]]["messages_sha256"]


@pytest.mark.parametrize("mutation", ["extra_scope_text", "scope_history_edit", "scope_role", "shared_debate", "missing_scope_suffix"])
def test_hashbound_message_intervention_rejects_changes_outside_exact_append(prepared, mutation):
    p = prepared
    calls, packets = copy.deepcopy(p.calls), copy.deepcopy(p.packets)
    scoped = packets["unit-0:scope_qwen_history"]["messages"]
    if mutation == "extra_scope_text": scoped[0]["content"] += " Extra directive."
    elif mutation == "scope_history_edit": scoped[-1]["content"] = "ORACLE: NO"
    elif mutation == "scope_role": scoped[0]["role"] = "user"
    elif mutation == "shared_debate":
        # Preserve the correct ordinary/scope difference but alter the shared debate
        # in both donor arms. Their common prefix must still match the empty control.
        packets["unit-0:ordinary_qwen_history"]["messages"][1]["content"] += " Added debate fact."
        scoped[1]["content"] += " Added debate fact."
    else: scoped[0]["content"] = packets["unit-0:ordinary_qwen_history"]["messages"][0]["content"]
    rehash_packets(calls, packets)
    rewrite_bound_panel(p, calls, packets)
    with pytest.raises(ValueError, match="Scope pair|prefix"):
        r.load_panel(p.inputs, p.auth)


@pytest.mark.parametrize("field,value", [("context_arm", "empty"), ("prompt_variant", "scope")])
def test_arm_factors_are_checked_instead_of_trusting_local_metadata(prepared, field, value):
    p = prepared
    calls = copy.deepcopy(p.calls)
    target = next(c for c in calls if c["arm"] == "ordinary_qwen_history")
    target[field] = value
    rewrite_bound_panel(p, calls, p.packets)
    with pytest.raises(ValueError, match="prompt/context"):
        r.load_panel(p.inputs, p.auth)


def test_all_six_conditions_are_matched_but_fresh_per_model(prepared):
    calls, packets, _ = r.load_panel(prepared.inputs, prepared.auth)
    assert len(calls) == 12 and len(packets) == 6
    assert r.Store is r.recipients.Store
    assert r.execute is r.recovery.execute
    for judge in r.MODELS:
        rows = [c for c in calls if c["judge"] == judge]
        assert len({c["cell_id"] for c in rows}) == 6
        assert len({c["seed"] for c in rows}) == 1
        assert {c["arm"] for c in rows} == set(r.ARMS)
    for arm in r.ARMS:
        assert len({c["packet_id"] for c in calls if c["arm"] == arm}) == 1


def test_synthetic_preflight_covers_scoped_answered_and_blocked_transport_only():
    calls, packets = r.synthetic_panel()
    assert len(calls) == 6
    for judge in r.MODELS:
        rows = [c for c in calls if c["judge"] == judge]
        assert {c["arm"] for c in rows} == {"ordinary_empty", "scope_qwen_history", "scope_llama_history"}
        for call in rows:
            packet = packets[call["packet_id"]]
            assert packet["messages"][0]["content"].endswith(r.scope_suffix()) == (call["prompt_variant"] == "scope")
            assert call["messages_sha256"] == r.recovery.digest(packet["messages"])
            assert set(r.request_kwargs(call, packet)) == {"model", "messages", "temperature", "max_tokens", "seed"}
    assert any("Query blocked" in r.canonical(p) for p in packets.values())


@pytest.mark.parametrize("field,value", [("schema_version", "phase4b_execution_v1"), ("stage", "4B-recipient"),
                                          ("run_id", "another-run"), ("paid_execution_authorized", "true")])
def test_wrong_run_or_nonboolean_grant_refuses_before_mutation(prepared, monkeypatch, field, value):
    p = prepared
    p.auth[field] = value
    r.atomic_json(p.authorization, p.auth)
    monkeypatch.setattr(r, "Store", lambda *a, **kw: pytest.fail("Unexpected store construction"))
    monkeypatch.setattr(r, "Provider", lambda: pytest.fail("Unexpected SDK construction"))
    with pytest.raises(ValueError):
        r.main(args(p, "run"))
    assert not p.output.exists()


def test_missing_main_cell_refuses_even_with_updated_artifact_hashes(prepared):
    p = prepared
    rewrite_bound_panel(p, p.calls[:-1], p.packets)
    with pytest.raises(ValueError, match="panel size"):
        r.load_panel(p.inputs, p.auth)


def test_source_bindings_include_the_imported_runtime_closure():
    assert {"rejudge/phase5_runner.py", "rejudge/phase4b_recipient_runner.py", "rejudge/phase4b_runner.py",
            "rejudge/phase4_runner.py", "rejudge/phase4b_labels.py", "rejudge/api_client.py",
            "rejudge/phase2_role_limits.py", "rejudge/phase2_plan.py", "rejudge/phase2_provider_price_snapshot.py",
            "rejudge/parsers.py", "analysis/infra/parsing.py", "rejudge/durable_fs.py"} <= set(r.SOURCE_FILES)
    assert all((r.ROOT / name).is_file() for name in r.SOURCE_FILES)


@pytest.mark.parametrize("field,value", [("uncertain_cap_usd", 25.01), ("preflight_cap_usd", 5.01)])
def test_frozen_subcaps_cannot_be_raised_by_execution_config(prepared, field, value):
    p = prepared
    p.auth[field] = value
    r.atomic_json(p.authorization, p.auth)
    with pytest.raises(ValueError, match="frozen Phase 5 ceilings"):
        r.verify_authorization(p.authorization, p.inputs, p.output)
    assert not p.output.exists()
